...
method1_df = pd.DataFrame([
    ...
    ["net_all_ports_per_year_elevator (t/yr net)", net_all_ports_per_year_elevator],
], columns=["metric", "value"])

summary_df = pd.DataFrame([
    ["Method1 net throughput (t/yr)", net_all_ports_per_year_elevator],
...
# Method 1 port constraint: elevator cap only
cap_port_elevator = U / kappa_G  # net tons/year/port
...
total_cost = float(method1_cost_per_t_net * m.sum()
                   + np.dot(df_bases["cost_per_t_net_USD"].to_numpy(dtype=float), n))
total_objective = float(total_cost + beta * T)
...
print("sanity T(3 ports only) =", M / (3 * cap_port_elevator))
print("Method2 net throughput (t/yr) =", net_method2_per_year)
print("Total throughput if all used at cap (t/yr) =", 3 * cap_port_elevator + net_method2_per_year)
print("sanity T(all caps) =", M / (3 * cap_port_elevator + net_method2_per_year))
...
# ----------------------------
# Method 1 only (3 ports, elevator cap)
# ----------------------------
rate_m1_channels = np.array([cap_port_elevator] * num_ports, dtype=float)
...
# ----------------------------
# Method 2 only (all bases at cap)
# ----------------------------
...
# ----------------------------
# Method 3 (LP mix) — evenly spread over T
# ----------------------------
if T <= 0:
    raise ValueError("LP solution T must be > 0")

rate_m3_ports = (m / T).astype(float)
rate_m3_bases = (n / T).astype(float)
...
# Optional: cumulative delivered sanity check
plt.figure()
