...
目标是最小化成本，并考虑时间限制，采用 **Pareto 最优前沿**，并引入 **工期压力惩罚项**：

\[
pressure = \frac{T_{\text{MAX}} - T}{T_{\text{MAX}} - T_{\text{MIN}}}
\]
\[
final\_cost = raw\_cost \cdot \left(1 + 0.5 \cdot (pressure^2)\right)
\]
